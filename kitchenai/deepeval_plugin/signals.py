...
def answer_relevance_post_save(sender, instance, **kwargs):
    ...
    if setting.is_answer_relevance_enabled:
        if kwargs.get("error"):
            return
        run_django_q_task('kitchenai.deepeval_plugin.tasks.run_answer_relevance', instance.id)
...
def faithfulness_post_save(sender, instance, **kwargs):
    ...
    if setting.is_faithfulness_enabled:
        if kwargs.get("error"):
            return
        run_django_q_task('kitchenai.deepeval_plugin.tasks.run_faithfulness', instance.id)
...
def contextual_relevancy_post_save(sender, instance, **kwargs):
    ...
    if setting.is_contextual_relevancy_enabled:
        if kwargs.get("error"):
            return
        run_django_q_task('kitchenai.deepeval_plugin.tasks.run_contextual_relevancy', instance.id)
...
def hallucination_post_save(sender, instance, **kwargs):
    ...
    if setting.is_hallucination_enabled:
        if kwargs.get("error"):
            return
        run_django_q_task('kitchenai.deepeval_plugin.tasks.run_hallucination', instance.id)
...
def toxicity_post_save(sender, instance, **kwargs):
    ...
    if setting.is_toxicity_enabled:
        if kwargs.get("error"):
            return
        run_django_q_task('kitchenai.deepeval_plugin.tasks.run_toxicity', instance.id)
